...
func validateFunction(g *Generator, i *validateFunctionInput) {
	...
	if !i.needsInput {
		...
		testResult = Err().Op("!=").Qual(generators.FastlyPackagePath, "ErrMissingServiceID")
		generateError = Id("t").Dot("Fatalf").Call(Lit("expected '%s', got: '%s'"), Qual(generators.FastlyPackagePath, "ErrMissingServiceID"), Err())
		body = append(body, For(List(Id("_"), Id("tc")).Op(":=").Range().Qual(g.base.Package.PkgPath, i.testCaseMap).Index(Lit("valid"))).Block(If(getResult, testResult).Block(generateError)))
		...
		// input structure.
		getResult = List(returnVals...).Op(":=").Qual(g.base.Package.PkgPath, i.method).Call(Qual(generators.FastlyPackagePath, "TestClient"), Qual(generators.FastlyPackagePath, "TestDeliveryServiceID"), Op("&").Id("tc").Dot("Input"))
		...
		body = append(body, For(List(Id("_"), Id("tc")).Op(":=").Range().Qual(g.base.Package.PkgPath, i.testCaseMap).Index(Lit("invalid"))).Block(getResult, If(testResult).Block(generateError)))
	}
