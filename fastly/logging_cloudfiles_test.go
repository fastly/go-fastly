...
func TestClient_Cloudfiles_Compute(t *testing.T) {
	t.Parallel()

	var err error
	var tv *Version
	Record(t, "cloudfiles/compute/version", func(c *Client) {
		tv = testVersionCompute(t, c)
	})

	// Create
	var cloudfilesCreateResp1, cloudfilesCreateResp2, cloudfilesCreateResp3 *Cloudfiles
	Record(t, "cloudfiles/compute/create", func(c *Client) {
	...
	Record(t, "cloudfiles/compute/create4", func(c *Client) {
		_, err = c.CreateCloudfiles(context.TODO(), &CreateCloudfilesInput{
			ServiceID:        TestComputeServiceID,
			ServiceVersion:   *tv.Number,
			Name:             new("test-cloudfiles-4"),
			User:             new("user"),
			AccessKey:        new("secret-key"),
			BucketName:       new("bucket-name"),
			Path:             new("/path"),
	...
	// Ensure deleted
	defer func() {
		Record(t, "cloudfiles/compute/cleanup", func(c *Client) {
	...
	// List
	var lc []*Cloudfiles
	Record(t, "cloudfiles/compute/list", func(c *Client) {
	...
	Record(t, "cloudfiles/compute/get", func(c *Client) {
	...
	Record(t, "cloudfiles/compute/update", func(c *Client) {
		cloudfilesUpdateResp1, err = c.UpdateCloudfiles(context.TODO(), &UpdateCloudfilesInput{
			ServiceID:        TestComputeServiceID,
			ServiceVersion:   *tv.Number,
			Name:             "test-cloudfiles",
			NewName:          new("new-test-cloudfiles"),
			User:             new("new-user"),
			Period:           new(0),
			FormatVersion:    new(2),
	...
	// Delete
	Record(t, "cloudfiles/compute/delete", func(c *Client) {
