package fastly

import (
	"context"
	"errors"
	"testing"
)

func TestClient_S3s(t *testing.T) {
	t.Parallel()

	var err error
	var tv *Version
	Record(t, "s3s/version", func(c *Client) {
		tv = testVersion(t, c)
	})

	// Create
	//
	// NOTE: You can't send the API and empty ResponseCondition.
	var s3CreateResp1, s3CreateResp2, s3CreateResp3, s3CreateResp4 *S3
	Record(t, "s3s/create", func(c *Client) {
		s3CreateResp1, err = c.CreateS3(context.TODO(), &CreateS3Input{
			ServiceID:                    TestDeliveryServiceID,
			ServiceVersion:               *tv.Number,
			Name:                         new("test-s3"),
			BucketName:                   new("bucket-name"),
			Domain:                       new("s3.us-east-1.amazonaws.com"),
			AccessKey:                    new("AKIAIOSFODNN7EXAMPLE"),
			SecretKey:                    new("wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY"),
			Path:                         new("/path"),
			Period:                       new(12),
			CompressionCodec:             new("snappy"),
			Format:                       new("format"),
			FormatVersion:                new(2),
			TimestampFormat:              new("%Y"),
			MessageType:                  new("classic"),
			Redundancy:                   new(S3RedundancyReduced),
			Placement:                    new("none"),
			PublicKey:                    new(pgpPublicKey()),
			ServerSideEncryptionKMSKeyID: new("1234"),
			ServerSideEncryption:         new(S3ServerSideEncryptionKMS),
			ACL:                          new(S3AccessControlListPrivate),
			FileMaxBytes:                 new(MiB),
		})
	})
	if err != nil {
		t.Fatal(err)
	}

	Record(t, "s3s/create2", func(c *Client) {
		s3CreateResp2, err = c.CreateS3(context.TODO(), &CreateS3Input{
			ServiceID:                    TestDeliveryServiceID,
			ServiceVersion:               *tv.Number,
			Name:                         new("test-s3-2"),
			BucketName:                   new("bucket-name"),
			Domain:                       new("s3.us-east-1.amazonaws.com"),
			AccessKey:                    new("AKIAIOSFODNN7EXAMPLE"),
			SecretKey:                    new("wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY"),
			Path:                         new("/path"),
			Period:                       new(12),
			GzipLevel:                    new(8),
			Format:                       new("format"),
			FormatVersion:                new(2),
			TimestampFormat:              new("%Y"),
			MessageType:                  new("classic"),
			Redundancy:                   new(S3RedundancyOneZoneIA),
			Placement:                    new("none"),
			PublicKey:                    new(pgpPublicKey()),
			ServerSideEncryptionKMSKeyID: new("1234"),
			ServerSideEncryption:         new(S3ServerSideEncryptionKMS),
			ACL:                          new(S3AccessControlListAuthenticatedRead),
			FileMaxBytes:                 new(10 * MiB),
		})
	})
	if err != nil {
		t.Fatal(err)
	}
	Record(t, "s3s/create3", func(c *Client) {
		s3CreateResp3, err = c.CreateS3(context.TODO(), &CreateS3Input{
			ServiceID:                    TestDeliveryServiceID,
			ServiceVersion:               *tv.Number,
			Name:                         new("test-s3-3"),
			BucketName:                   new("bucket-name"),
			Domain:                       new("s3.us-east-1.amazonaws.com"),
			AccessKey:                    new("AKIAIOSFODNN7EXAMPLE"),
			SecretKey:                    new("wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY"),
			Path:                         new("/path"),
			Period:                       new(12),
			Format:                       new("format"),
			CompressionCodec:             new("snappy"),
			FormatVersion:                new(2),
			TimestampFormat:              new("%Y"),
			MessageType:                  new("classic"),
			Redundancy:                   new(S3RedundancyStandardIA),
			Placement:                    new("none"),
			PublicKey:                    new(pgpPublicKey()),
			ServerSideEncryptionKMSKeyID: new("1234"),
			ServerSideEncryption:         new(S3ServerSideEncryptionKMS),
			ACL:                          new(S3AccessControlListBucketOwnerFullControl),
		})
	})
	if err != nil {
		t.Fatal(err)
	}

	Record(t, "s3s/create4", func(c *Client) {
		s3CreateResp4, err = c.CreateS3(context.TODO(), &CreateS3Input{
			ServiceID:                    TestDeliveryServiceID,
			ServiceVersion:               *tv.Number,
			Name:                         new("test-s3-4"),
			BucketName:                   new("bucket-name"),
			Domain:                       new("s3.us-east-1.amazonaws.com"),
			IAMRole:                      new("arn:aws:iam::123456789012:role/S3Access"),
			Path:                         new("/path"),
			Period:                       new(12),
			Format:                       new("format"),
			CompressionCodec:             new("snappy"),
			FormatVersion:                new(2),
			TimestampFormat:              new("%Y"),
			MessageType:                  new("classic"),
			Placement:                    new("none"),
			Redundancy:                   new(S3RedundancyStandard),
			PublicKey:                    new(pgpPublicKey()),
			ServerSideEncryptionKMSKeyID: new("1234"),
			ServerSideEncryption:         new(S3ServerSideEncryptionKMS),
			FileMaxBytes:                 new(10 * MiB),
		})
	})
	if err != nil {
		t.Fatal(err)
	}

	// This case is expected to fail
	Record(t, "s3s/create5", func(c *Client) {
		_, err = c.CreateS3(context.TODO(), &CreateS3Input{
			ServiceID:                    TestDeliveryServiceID,
			ServiceVersion:               *tv.Number,
			Name:                         new("test-s3"),
			BucketName:                   new("bucket-name"),
			Domain:                       new("s3.us-east-1.amazonaws.com"),
			AccessKey:                    new("AKIAIOSFODNN7EXAMPLE"),
			SecretKey:                    new("wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY"),
			IAMRole:                      new("arn:aws:iam::123456789012:role/S3Access"),
			Path:                         new("/path"),
			Period:                       new(12),
			CompressionCodec:             new("snappy"),
			Format:                       new("format"),
			FormatVersion:                new(2),
			TimestampFormat:              new("%Y"),
			MessageType:                  new("classic"),
			Redundancy:                   new(S3RedundancyReduced),
			Placement:                    new("none"),
			PublicKey:                    new(pgpPublicKey()),
			ServerSideEncryptionKMSKeyID: new("1234"),
			ServerSideEncryption:         new(S3ServerSideEncryptionKMS),
		})
	})
	if err == nil {
		t.Fatal(err)
	}

	// This case is expected to fail
	Record(t, "s3s/create6", func(c *Client) {
		_, err = c.CreateS3(context.TODO(), &CreateS3Input{
			ServiceID:                    TestDeliveryServiceID,
			ServiceVersion:               *tv.Number,
			Name:                         new("test-s3"),
			BucketName:                   new("bucket-name"),
			Domain:                       new("s3.us-east-1.amazonaws.com"),
			IAMRole:                      new("badarn"),
			Path:                         new("/path"),
			Period:                       new(12),
			CompressionCodec:             new("snappy"),
			Format:                       new("format"),
			FormatVersion:                new(2),
			TimestampFormat:              new("%Y"),
			MessageType:                  new("classic"),
			Redundancy:                   new(S3RedundancyReduced),
			Placement:                    new("none"),
			PublicKey:                    new(pgpPublicKey()),
			ServerSideEncryptionKMSKeyID: new("1234"),
			ServerSideEncryption:         new(S3ServerSideEncryptionKMS),
		})
	})
	if err == nil {
		t.Fatal(err)
	}

	// This case is expected to fail because both CompressionCodec and
	// GzipLevel are present.
	Record(t, "s3s/create7", func(c *Client) {
		_, err = c.CreateS3(context.TODO(), &CreateS3Input{
			ServiceID:                    TestDeliveryServiceID,
			ServiceVersion:               *tv.Number,
			Name:                         new("test-s3-2"),
			BucketName:                   new("bucket-name"),
			Domain:                       new("s3.us-east-1.amazonaws.com"),
			AccessKey:                    new("AKIAIOSFODNN7EXAMPLE"),
			SecretKey:                    new("wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY"),
			Path:                         new("/path"),
			Period:                       new(12),
			CompressionCodec:             new("snappy"),
			GzipLevel:                    new(8),
			Format:                       new("format"),
			FormatVersion:                new(2),
			TimestampFormat:              new("%Y"),
			MessageType:                  new("classic"),
			Redundancy:                   new(S3RedundancyReduced),
			Placement:                    new("none"),
			PublicKey:                    new(pgpPublicKey()),
			ServerSideEncryptionKMSKeyID: new("1234"),
			ServerSideEncryption:         new(S3ServerSideEncryptionKMS),
		})
	})
	if err == nil {
		t.Fatal(err)
	}

	// Ensure deleted
	defer func() {
		Record(t, "s3s/cleanup", func(c *Client) {
			_ = c.DeleteS3(context.TODO(), &DeleteS3Input{
				ServiceID:      TestDeliveryServiceID,
				ServiceVersion: *tv.Number,
				Name:           "test-s3",
			})

			_ = c.DeleteS3(context.TODO(), &DeleteS3Input{
				ServiceID:      TestDeliveryServiceID,
				ServiceVersion: *tv.Number,
				Name:           "test-s3-3",
			})

			_ = c.DeleteS3(context.TODO(), &DeleteS3Input{
				ServiceID:      TestDeliveryServiceID,
				ServiceVersion: *tv.Number,
				Name:           "test-s3-4",
			})

			_ = c.DeleteS3(context.TODO(), &DeleteS3Input{
				ServiceID:      TestDeliveryServiceID,
				ServiceVersion: *tv.Number,
				Name:           "new-test-s3",
			})
		})
	}()

	if *s3CreateResp1.Name != "test-s3" {
		t.Errorf("bad name: %q", *s3CreateResp1.Name)
	}
	if *s3CreateResp1.BucketName != "bucket-name" {
		t.Errorf("bad bucket_name: %q", *s3CreateResp1.BucketName)
	}
	if *s3CreateResp1.AccessKey != "AKIAIOSFODNN7EXAMPLE" { //nolint: gosec
		t.Errorf("bad access_key: %q", *s3CreateResp1.AccessKey)
	}
	if *s3CreateResp1.SecretKey != "wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY" {
		t.Errorf("bad secret_key: %q", *s3CreateResp1.SecretKey)
	}
	if s3CreateResp1.IAMRole != nil {
		t.Errorf("bad iam_role: %q", *s3CreateResp1.IAMRole)
	}
	if *s3CreateResp1.Domain != "s3.us-east-1.amazonaws.com" {
		t.Errorf("bad domain: %q", *s3CreateResp1.Domain)
	}
	if *s3CreateResp1.Path != "/path" {
		t.Errorf("bad path: %q", *s3CreateResp1.Path)
	}
	if *s3CreateResp1.Period != 12 {
		t.Errorf("bad period: %d", *s3CreateResp1.Period)
	}
	if *s3CreateResp1.CompressionCodec != "snappy" {
		t.Errorf("bad compression_codec: %q", *s3CreateResp1.CompressionCodec)
	}
	if *s3CreateResp1.GzipLevel != 0 {
		t.Errorf("bad gzip_level: %d", *s3CreateResp1.GzipLevel)
	}
	if *s3CreateResp1.Format != "format" {
		t.Errorf("bad format: %q", *s3CreateResp1.Format)
	}
	if *s3CreateResp1.FormatVersion != 2 {
		t.Errorf("bad format_version: %d", *s3CreateResp1.FormatVersion)
	}
	if *s3CreateResp1.TimestampFormat != "%Y" {
		t.Errorf("bad timestamp_format: %q", *s3CreateResp1.TimestampFormat)
	}
	if *s3CreateResp1.Redundancy != S3RedundancyReduced {
		t.Errorf("bad redundancy: %q", *s3CreateResp1.Redundancy)
	}
	if *s3CreateResp1.MessageType != "classic" {
		t.Errorf("bad message_type: %q", *s3CreateResp1.MessageType)
	}
	if *s3CreateResp1.Placement != "none" {
		t.Errorf("bad placement: %q", *s3CreateResp1.Placement)
	}
	if *s3CreateResp1.ResponseCondition != "" {
		t.Errorf("bad response_condition: %q", *s3CreateResp1.ResponseCondition)
	}
	if *s3CreateResp1.PublicKey != pgpPublicKey() {
		t.Errorf("bad public_key: %q", *s3CreateResp1.PublicKey)
	}
	if *s3CreateResp1.ServerSideEncryption != S3ServerSideEncryptionKMS {
		t.Errorf("bad server_side_encryption: %q", *s3CreateResp1.ServerSideEncryption)
	}
	if *s3CreateResp1.ServerSideEncryptionKMSKeyID != "1234" {
		t.Errorf("bad server_side_encryption_kms_key_id: %q", *s3CreateResp1.ServerSideEncryptionKMSKeyID)
	}
	if *s3CreateResp1.ACL != S3AccessControlListPrivate {
		t.Errorf("bad acl: %s", *s3CreateResp1.ACL)
	}
	if *s3CreateResp1.FileMaxBytes != MiB {
		t.Errorf("bad file_max_bytes: %d", *s3CreateResp1.FileMaxBytes)
	}
	if *s3CreateResp2.FileMaxBytes != 10*MiB {
		t.Errorf("bad file_max_bytes: %d", *s3CreateResp2.FileMaxBytes)
	}
	if *s3CreateResp3.CompressionCodec != "snappy" {
		t.Errorf("bad compression_codec: %q", *s3CreateResp1.CompressionCodec)
	}
	if *s3CreateResp3.GzipLevel != 0 {
		t.Errorf("bad gzip_level: %d", *s3CreateResp1.GzipLevel)
	}
	if *s3CreateResp3.Redundancy != S3RedundancyStandardIA {
		t.Errorf("bad acl: %s", *s3CreateResp3.Redundancy)
	}
	if *s3CreateResp3.ACL != S3AccessControlListBucketOwnerFullControl {
		t.Errorf("bad acl: %s", *s3CreateResp3.ACL)
	}
	if *s3CreateResp3.FileMaxBytes != 0 {
		t.Errorf("bad file_max_bytes: %d", *s3CreateResp3.FileMaxBytes)
	}
	if s3CreateResp4.AccessKey != nil {
		t.Errorf("bad access_key: %q", *s3CreateResp4.AccessKey)
	}
	if s3CreateResp4.SecretKey != nil {
		t.Errorf("bad secret_key: %q", *s3CreateResp4.SecretKey)
	}
	if *s3CreateResp4.IAMRole != "arn:aws:iam::123456789012:role/S3Access" {
		t.Errorf("bad iam_role: %q", *s3CreateResp4.IAMRole)
	}
	if *s3CreateResp4.Redundancy != S3RedundancyStandard {
		t.Errorf("bad acl: %s", *s3CreateResp4.Redundancy)
	}
	if s3CreateResp4.ACL != nil {
		t.Errorf("bad acl: %s", *s3CreateResp4.ACL)
	}
	if *s3CreateResp4.FileMaxBytes != 10*MiB {
		t.Errorf("bad file_max_bytes: %d", *s3CreateResp4.FileMaxBytes)
	}

	// List
	var s3s []*S3
	Record(t, "s3s/list", func(c *Client) {
		s3s, err = c.ListS3s(context.TODO(), &ListS3sInput{
			ServiceID:      TestDeliveryServiceID,
			ServiceVersion: *tv.Number,
		})
	})
	if err != nil {
		t.Fatal(err)
	}
	if len(s3s) < 1 {
		t.Errorf("bad s3s: %v", s3s)
	}

	// Get
	var s3GetResp, s3GetResp2 *S3
	Record(t, "s3s/get", func(c *Client) {
		s3GetResp, err = c.GetS3(context.TODO(), &GetS3Input{
			ServiceID:      TestDeliveryServiceID,
			ServiceVersion: *tv.Number,
			Name:           "test-s3",
		})
	})
	if err != nil {
		t.Fatal(err)
	}

	// Request the configuration using the IAM role
	Record(t, "s3s/get2", func(c *Client) {
		s3GetResp2, err = c.GetS3(context.TODO(), &GetS3Input{
			ServiceID:      TestDeliveryServiceID,
			ServiceVersion: *tv.Number,
			Name:           "test-s3-4",
		})
	})
	if err != nil {
		t.Fatal(err)
	}

	if *s3CreateResp1.Name != *s3GetResp.Name {
		t.Errorf("bad name: %q", *s3CreateResp1.Name)
	}
	if *s3CreateResp1.BucketName != *s3GetResp.BucketName {
		t.Errorf("bad bucket_name: %q", *s3CreateResp1.BucketName)
	}
	if *s3CreateResp1.AccessKey != *s3GetResp.AccessKey {
		t.Errorf("bad access_key: %q", *s3CreateResp1.AccessKey)
	}
	if *s3CreateResp1.SecretKey != *s3GetResp.SecretKey {
		t.Errorf("bad secret_key: %q", *s3CreateResp1.SecretKey)
	}
	if s3CreateResp1.IAMRole != s3GetResp.IAMRole {
		t.Errorf("bad iam_role: %q", *s3CreateResp1.IAMRole)
	}
	if *s3CreateResp1.Domain != *s3GetResp.Domain {
		t.Errorf("bad domain: %q", *s3CreateResp1.Domain)
	}
	if *s3CreateResp1.Path != *s3GetResp.Path {
		t.Errorf("bad path: %q", *s3CreateResp1.Path)
	}
	if *s3CreateResp1.Period != *s3GetResp.Period {
		t.Errorf("bad period: %d", *s3CreateResp1.Period)
	}
	if *s3CreateResp1.CompressionCodec != *s3GetResp.CompressionCodec {
		t.Errorf("bad compression_codec: %q", *s3CreateResp1.CompressionCodec)
	}
	if *s3CreateResp1.GzipLevel != *s3GetResp.GzipLevel {
		t.Errorf("bad gzip_level: %d", *s3CreateResp1.GzipLevel)
	}
	if *s3CreateResp1.Format != *s3GetResp.Format {
		t.Errorf("bad format: %q", *s3CreateResp1.Format)
	}
	if *s3CreateResp1.FormatVersion != *s3GetResp.FormatVersion {
		t.Errorf("bad format_version: %d", *s3CreateResp1.FormatVersion)
	}
	if *s3CreateResp1.TimestampFormat != *s3GetResp.TimestampFormat {
		t.Errorf("bad timestamp_format: %q", *s3CreateResp1.TimestampFormat)
	}
	if *s3CreateResp1.Redundancy != *s3GetResp.Redundancy {
		t.Errorf("bad redundancy: %q", *s3CreateResp1.Redundancy)
	}
	if *s3CreateResp1.Placement != *s3GetResp.Placement {
		t.Errorf("bad placement: %q", *s3CreateResp1.Placement)
	}
	if *s3CreateResp1.ResponseCondition != "" {
		t.Errorf("bad response_condition: %q", *s3CreateResp1.ResponseCondition)
	}
	if *s3CreateResp1.PublicKey != pgpPublicKey() {
		t.Errorf("bad public_key: %q", *s3CreateResp1.PublicKey)
	}
	if *s3CreateResp1.ServerSideEncryption != *s3GetResp.ServerSideEncryption {
		t.Errorf("bad server_side_encryption: %q", *s3CreateResp1.ServerSideEncryption)
	}
	if *s3CreateResp1.ServerSideEncryptionKMSKeyID != *s3GetResp.ServerSideEncryptionKMSKeyID {
		t.Errorf("bad server_side_encryption_kms_key_id: %q", *s3CreateResp1.ServerSideEncryptionKMSKeyID)
	}
	if *s3CreateResp1.ACL != *s3GetResp.ACL {
		t.Errorf("bad acl: %s", *s3CreateResp1.ACL)
	}
	if s3CreateResp4.AccessKey != s3GetResp2.AccessKey {
		t.Errorf("bad access_key: %q", *s3CreateResp4.AccessKey)
	}
	if s3CreateResp4.SecretKey != s3GetResp2.SecretKey {
		t.Errorf("bad secret_key: %q", *s3CreateResp4.SecretKey)
	}
	if *s3CreateResp4.IAMRole != *s3GetResp2.IAMRole {
		t.Errorf("bad iam_role: %q", *s3CreateResp4.IAMRole)
	}
	if *s3CreateResp4.Redundancy != *s3GetResp2.Redundancy {
		t.Errorf("bad redundancy: %q", *s3CreateResp4.Redundancy)
	}
	if s3CreateResp4.ACL != s3GetResp2.ACL {
		t.Errorf("bad acl: %s", *s3CreateResp4.ACL)
	}

	// Update
	var s3UpdateResp1, s3UpdateResp2, s3UpdateResp3, s3UpdateResp4, s3UpdateResp5 *S3
	Record(t, "s3s/update", func(c *Client) {
		s3UpdateResp1, err = c.UpdateS3(context.TODO(), &UpdateS3Input{
			ServiceID:        TestDeliveryServiceID,
			ServiceVersion:   *tv.Number,
			Name:             "test-s3",
			NewName:          new("new-test-s3"),
			PublicKey:        new(pgpPublicKeyUpdate()),
			CompressionCodec: new("zstd"),
			FileMaxBytes:     new(5 * MiB),
			ProcessingRegion: new("eu"),
			Placement:        NullValue[string](),
		})
	})
	if err != nil {
		t.Fatal(err)
	}
	if s3UpdateResp1.Placement != nil {
		t.Errorf("bad placement: %q", *s3UpdateResp1.Placement)
	}

	// Test that CompressionCodec can be set for a an endpoint where
	// GzipLevel was specified at creation time.
	Record(t, "s3s/update2", func(c *Client) {
		s3UpdateResp2, err = c.UpdateS3(context.TODO(), &UpdateS3Input{
			ServiceID:        TestDeliveryServiceID,
			ServiceVersion:   *tv.Number,
			Name:             "test-s3-2",
			CompressionCodec: new("zstd"),
			Placement:        NullValue[string](),
		})
	})
	if err != nil {
		t.Fatal(err)
	}
	if s3UpdateResp2.Placement != nil {
		t.Errorf("bad placement: %q", *s3UpdateResp2.Placement)
	}

	// Test that GzipLevel can be set for an endpoint where CompressionCodec
	// was set at creation time.
	Record(t, "s3s/update3", func(c *Client) {
		s3UpdateResp3, err = c.UpdateS3(context.TODO(), &UpdateS3Input{
			ServiceID:      TestDeliveryServiceID,
			ServiceVersion: *tv.Number,
			Name:           "test-s3-3",
			GzipLevel:      new(9),
			Placement:      NullValue[string](),
		})
	})
	if err != nil {
		t.Fatal(err)
	}
	if s3UpdateResp3.Placement != nil {
		t.Errorf("bad placement: %q", *s3UpdateResp3.Placement)
	}

	// Test that a configuration using an access key/secret key can be
	// updated to use IAM role.
	Record(t, "s3s/update4", func(c *Client) {
		s3UpdateResp4, err = c.UpdateS3(context.TODO(), &UpdateS3Input{
			ServiceID:      TestDeliveryServiceID,
			ServiceVersion: *tv.Number,
			Name:           "new-test-s3",
			AccessKey:      new(""),
			SecretKey:      new(""),
			IAMRole:        new("arn:aws:iam::123456789012:role/S3Access"),
			Placement:      NullValue[string](),
		})
	})
	if err != nil {
		t.Fatal(err)
	}
	if s3UpdateResp4.Placement != nil {
		t.Errorf("bad placement: %q", *s3UpdateResp4.Placement)
	}

	// Test that a configuration using an IAM role can be updated to use
	// access key/secret key.
	Record(t, "s3s/update5", func(c *Client) {
		s3UpdateResp5, err = c.UpdateS3(context.TODO(), &UpdateS3Input{
			ServiceID:      TestDeliveryServiceID,
			ServiceVersion: *tv.Number,
			Name:           "test-s3-4",
			AccessKey:      new("AKIAIOSFODNN7EXAMPLE"),
			SecretKey:      new("wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY"),
			IAMRole:        new(""),
			Placement:      NullValue[string](),
		})
	})
	if err != nil {
		t.Fatal(err)
	}
	if s3UpdateResp5.Placement != nil {
		t.Errorf("bad placement: %q", *s3UpdateResp5.Placement)
	}

	// Test that an invalid IAM role ARN is rejected. This case is expected
	// to fail.
	Record(t, "s3s/update6", func(c *Client) {
		_, err = c.UpdateS3(context.TODO(), &UpdateS3Input{
			ServiceID:      TestDeliveryServiceID,
			ServiceVersion: *tv.Number,
			Name:           "test-s3",
			IAMRole:        new("badarn"),
			Placement:      NullValue[string](),
		})
	})
	if err == nil {
		t.Fatal(err)
	}

	if *s3UpdateResp1.Name != "new-test-s3" {
		t.Errorf("bad name: %q", *s3UpdateResp1.Name)
	}
	if *s3UpdateResp1.PublicKey != pgpPublicKeyUpdate() {
		t.Errorf("bad public_key: %q", *s3UpdateResp1.PublicKey)
	}
	if *s3UpdateResp1.CompressionCodec != "zstd" {
		t.Errorf("bad compression_codec: %q", *s3UpdateResp1.CompressionCodec)
	}
	if s3UpdateResp1.GzipLevel != nil {
		t.Errorf("bad gzip_level: %d", *s3UpdateResp1.GzipLevel)
	}
	if *s3UpdateResp1.FileMaxBytes != 5*MiB {
		t.Errorf("bad file_max_bytes: %d", *s3UpdateResp1.FileMaxBytes)
	}
	if *s3UpdateResp1.ProcessingRegion != "eu" {
		t.Errorf("bad log_processing_region: %q", *s3UpdateResp1.ProcessingRegion)
	}
	if *s3UpdateResp2.CompressionCodec != "zstd" {
		t.Errorf("bad compression_codec: %q", *s3UpdateResp2.CompressionCodec)
	}
	if *s3UpdateResp2.GzipLevel != 0 {
		t.Errorf("bad gzip_level: %d", *s3UpdateResp2.GzipLevel)
	}
	if s3UpdateResp3.CompressionCodec != nil {
		t.Errorf("bad compression_codec: %q", *s3UpdateResp3.CompressionCodec)
	}
	if *s3UpdateResp3.GzipLevel != 9 {
		t.Errorf("bad gzip_level: %d", *s3UpdateResp3.GzipLevel)
	}
	if *s3UpdateResp4.AccessKey != "" {
		t.Errorf("bad access_key: %q", *s3UpdateResp4.AccessKey)
	}
	if *s3UpdateResp4.SecretKey != "" {
		t.Errorf("bad secret_key: %q", *s3UpdateResp4.SecretKey)
	}
	if *s3UpdateResp4.IAMRole != "arn:aws:iam::123456789012:role/S3Access" {
		t.Errorf("bad iam_role: %q", *s3UpdateResp4.IAMRole)
	}
	if *s3UpdateResp5.AccessKey != "AKIAIOSFODNN7EXAMPLE" { //nolint: gosec
		t.Errorf("bad access_key: %q", *s3UpdateResp5.AccessKey)
	}
	if *s3UpdateResp5.SecretKey != "wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY" {
		t.Errorf("bad secret_key: %q", *s3UpdateResp5.SecretKey)
	}
	if *s3UpdateResp5.IAMRole != "" {
		t.Errorf("bad iam_role: %q", *s3UpdateResp5.IAMRole)
	}

	// Delete
	Record(t, "s3s/delete", func(c *Client) {
		err = c.DeleteS3(context.TODO(), &DeleteS3Input{
			ServiceID:      TestDeliveryServiceID,
			ServiceVersion: *tv.Number,
			Name:           "new-test-s3",
		})
	})
	if err != nil {
		t.Fatal(err)
	}
}

func TestClient_S3s_Compute(t *testing.T) {
	t.Parallel()

	var err error
	var tv *Version
	Record(t, "s3s/compute/version", func(c *Client) {
		tv = testVersionCompute(t, c)
	})

	// Create
	//
	// NOTE: You can't send the API and empty ResponseCondition.
	var s3CreateResp1, s3CreateResp2, s3CreateResp3, s3CreateResp4 *S3
	Record(t, "s3s/compute/create", func(c *Client) {
		s3CreateResp1, err = c.CreateS3(context.TODO(), &CreateS3Input{
			ServiceID:                    TestComputeServiceID,
			ServiceVersion:               *tv.Number,
			Name:                         new("test-s3"),
			BucketName:                   new("bucket-name"),
			Domain:                       new("s3.us-east-1.amazonaws.com"),
			AccessKey:                    new("AKIAIOSFODNN7EXAMPLE"),
			SecretKey:                    new("wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY"),
			Path:                         new("/path"),
			Period:                       new(12),
			CompressionCodec:             new("snappy"),
			Format:                       new("format"),
			FormatVersion:                new(2),
			TimestampFormat:              new("%Y"),
			MessageType:                  new("classic"),
			Redundancy:                   new(S3RedundancyReduced),
			Placement:                    new("none"),
			PublicKey:                    new(pgpPublicKey()),
			ServerSideEncryptionKMSKeyID: new("1234"),
			ServerSideEncryption:         new(S3ServerSideEncryptionKMS),
			ACL:                          new(S3AccessControlListPrivate),
			FileMaxBytes:                 new(MiB),
		})
	})
	if err != nil {
		t.Fatal(err)
	}

	Record(t, "s3s/compute/create2", func(c *Client) {
		s3CreateResp2, err = c.CreateS3(context.TODO(), &CreateS3Input{
			ServiceID:                    TestComputeServiceID,
			ServiceVersion:               *tv.Number,
			Name:                         new("test-s3-2"),
			BucketName:                   new("bucket-name"),
			Domain:                       new("s3.us-east-1.amazonaws.com"),
			AccessKey:                    new("AKIAIOSFODNN7EXAMPLE"),
			SecretKey:                    new("wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY"),
			Path:                         new("/path"),
			Period:                       new(12),
			GzipLevel:                    new(8),
			Format:                       new("format"),
			FormatVersion:                new(2),
			TimestampFormat:              new("%Y"),
			MessageType:                  new("classic"),
			Redundancy:                   new(S3RedundancyOneZoneIA),
			Placement:                    new("none"),
			PublicKey:                    new(pgpPublicKey()),
			ServerSideEncryptionKMSKeyID: new("1234"),
			ServerSideEncryption:         new(S3ServerSideEncryptionKMS),
			ACL:                          new(S3AccessControlListAuthenticatedRead),
			FileMaxBytes:                 new(10 * MiB),
		})
	})
	if err != nil {
		t.Fatal(err)
	}
	Record(t, "s3s/compute/create3", func(c *Client) {
		s3CreateResp3, err = c.CreateS3(context.TODO(), &CreateS3Input{
			ServiceID:                    TestComputeServiceID,
			ServiceVersion:               *tv.Number,
			Name:                         new("test-s3-3"),
			BucketName:                   new("bucket-name"),
			Domain:                       new("s3.us-east-1.amazonaws.com"),
			AccessKey:                    new("AKIAIOSFODNN7EXAMPLE"),
			SecretKey:                    new("wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY"),
			Path:                         new("/path"),
			Period:                       new(12),
			Format:                       new("format"),
			CompressionCodec:             new("snappy"),
			FormatVersion:                new(2),
			TimestampFormat:              new("%Y"),
			MessageType:                  new("classic"),
			Redundancy:                   new(S3RedundancyStandardIA),
			Placement:                    new("none"),
			PublicKey:                    new(pgpPublicKey()),
			ServerSideEncryptionKMSKeyID: new("1234"),
			ServerSideEncryption:         new(S3ServerSideEncryptionKMS),
			ACL:                          new(S3AccessControlListBucketOwnerFullControl),
		})
	})
	if err != nil {
		t.Fatal(err)
	}

	Record(t, "s3s/compute/create4", func(c *Client) {
		s3CreateResp4, err = c.CreateS3(context.TODO(), &CreateS3Input{
			ServiceID:                    TestComputeServiceID,
			ServiceVersion:               *tv.Number,
			Name:                         new("test-s3-4"),
			BucketName:                   new("bucket-name"),
			Domain:                       new("s3.us-east-1.amazonaws.com"),
			IAMRole:                      new("arn:aws:iam::123456789012:role/S3Access"),
			Path:                         new("/path"),
			Period:                       new(12),
			Format:                       new("format"),
			CompressionCodec:             new("snappy"),
			FormatVersion:                new(2),
			TimestampFormat:              new("%Y"),
			MessageType:                  new("classic"),
			Placement:                    new("none"),
			Redundancy:                   new(S3RedundancyStandard),
			PublicKey:                    new(pgpPublicKey()),
			ServerSideEncryptionKMSKeyID: new("1234"),
			ServerSideEncryption:         new(S3ServerSideEncryptionKMS),
			FileMaxBytes:                 new(10 * MiB),
		})
	})
	if err != nil {
		t.Fatal(err)
	}

	// This case is expected to fail
	Record(t, "s3s/compute/create5", func(c *Client) {
		_, err = c.CreateS3(context.TODO(), &CreateS3Input{
			ServiceID:                    TestComputeServiceID,
			ServiceVersion:               *tv.Number,
			Name:                         new("test-s3"),
			BucketName:                   new("bucket-name"),
			Domain:                       new("s3.us-east-1.amazonaws.com"),
			AccessKey:                    new("AKIAIOSFODNN7EXAMPLE"),
			SecretKey:                    new("wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY"),
			IAMRole:                      new("arn:aws:iam::123456789012:role/S3Access"),
			Path:                         new("/path"),
			Period:                       new(12),
			CompressionCodec:             new("snappy"),
			Format:                       new("format"),
			FormatVersion:                new(2),
			TimestampFormat:              new("%Y"),
			MessageType:                  new("classic"),
			Redundancy:                   new(S3RedundancyReduced),
			Placement:                    new("none"),
			PublicKey:                    new(pgpPublicKey()),
			ServerSideEncryptionKMSKeyID: new("1234"),
			ServerSideEncryption:         new(S3ServerSideEncryptionKMS),
		})
	})
	if err == nil {
		t.Fatal(err)
	}

	// This case is expected to fail
	Record(t, "s3s/compute/create6", func(c *Client) {
		_, err = c.CreateS3(context.TODO(), &CreateS3Input{
			ServiceID:                    TestComputeServiceID,
			ServiceVersion:               *tv.Number,
			Name:                         new("test-s3"),
			BucketName:                   new("bucket-name"),
			Domain:                       new("s3.us-east-1.amazonaws.com"),
			IAMRole:                      new("badarn"),
			Path:                         new("/path"),
			Period:                       new(12),
			CompressionCodec:             new("snappy"),
			Format:                       new("format"),
			FormatVersion:                new(2),
			TimestampFormat:              new("%Y"),
			MessageType:                  new("classic"),
			Redundancy:                   new(S3RedundancyReduced),
			Placement:                    new("none"),
			PublicKey:                    new(pgpPublicKey()),
			ServerSideEncryptionKMSKeyID: new("1234"),
			ServerSideEncryption:         new(S3ServerSideEncryptionKMS),
		})
	})
	if err == nil {
		t.Fatal(err)
	}

	// This case is expected to fail because both CompressionCodec and
	// GzipLevel are present.
	Record(t, "s3s/compute/create7", func(c *Client) {
		_, err = c.CreateS3(context.TODO(), &CreateS3Input{
			ServiceID:                    TestComputeServiceID,
			ServiceVersion:               *tv.Number,
			Name:                         new("test-s3-2"),
			BucketName:                   new("bucket-name"),
			Domain:                       new("s3.us-east-1.amazonaws.com"),
			AccessKey:                    new("AKIAIOSFODNN7EXAMPLE"),
			SecretKey:                    new("wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY"),
			Path:                         new("/path"),
			Period:                       new(12),
			CompressionCodec:             new("snappy"),
			GzipLevel:                    new(8),
			Format:                       new("format"),
			FormatVersion:                new(2),
			TimestampFormat:              new("%Y"),
			MessageType:                  new("classic"),
			Redundancy:                   new(S3RedundancyReduced),
			Placement:                    new("none"),
			PublicKey:                    new(pgpPublicKey()),
			ServerSideEncryptionKMSKeyID: new("1234"),
			ServerSideEncryption:         new(S3ServerSideEncryptionKMS),
		})
	})
	if err == nil {
		t.Fatal(err)
	}

	// Ensure deleted
	defer func() {
		Record(t, "s3s/compute/cleanup", func(c *Client) {
			_ = c.DeleteS3(context.TODO(), &DeleteS3Input{
				ServiceID:      TestComputeServiceID,
				ServiceVersion: *tv.Number,
				Name:           "test-s3",
			})

			_ = c.DeleteS3(context.TODO(), &DeleteS3Input{
				ServiceID:      TestComputeServiceID,
				ServiceVersion: *tv.Number,
				Name:           "test-s3-3",
			})

			_ = c.DeleteS3(context.TODO(), &DeleteS3Input{
				ServiceID:      TestComputeServiceID,
				ServiceVersion: *tv.Number,
				Name:           "test-s3-4",
			})

			_ = c.DeleteS3(context.TODO(), &DeleteS3Input{
				ServiceID:      TestComputeServiceID,
				ServiceVersion: *tv.Number,
				Name:           "new-test-s3",
			})
		})
	}()

	if *s3CreateResp1.Name != "test-s3" {
		t.Errorf("bad name: %q", *s3CreateResp1.Name)
	}
	if *s3CreateResp1.BucketName != "bucket-name" {
		t.Errorf("bad bucket_name: %q", *s3CreateResp1.BucketName)
	}
	if *s3CreateResp1.AccessKey != "AKIAIOSFODNN7EXAMPLE" { //nolint: gosec
		t.Errorf("bad access_key: %q", *s3CreateResp1.AccessKey)
	}
	if *s3CreateResp1.SecretKey != "wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY" {
		t.Errorf("bad secret_key: %q", *s3CreateResp1.SecretKey)
	}
	if s3CreateResp1.IAMRole != nil {
		t.Errorf("bad iam_role: %q", *s3CreateResp1.IAMRole)
	}
	if *s3CreateResp1.Domain != "s3.us-east-1.amazonaws.com" {
		t.Errorf("bad domain: %q", *s3CreateResp1.Domain)
	}
	if *s3CreateResp1.Path != "/path" {
		t.Errorf("bad path: %q", *s3CreateResp1.Path)
	}
	if *s3CreateResp1.Period != 12 {
		t.Errorf("bad period: %d", *s3CreateResp1.Period)
	}
	if *s3CreateResp1.CompressionCodec != "snappy" {
		t.Errorf("bad compression_codec: %q", *s3CreateResp1.CompressionCodec)
	}
	if *s3CreateResp1.GzipLevel != 0 {
		t.Errorf("bad gzip_level: %d", *s3CreateResp1.GzipLevel)
	}
	if *s3CreateResp1.Format != "format" {
		t.Errorf("bad format: %q", *s3CreateResp1.Format)
	}
	if *s3CreateResp1.FormatVersion != 2 {
		t.Errorf("bad format_version: %d", *s3CreateResp1.FormatVersion)
	}
	if *s3CreateResp1.TimestampFormat != "%Y" {
		t.Errorf("bad timestamp_format: %q", *s3CreateResp1.TimestampFormat)
	}
	if *s3CreateResp1.Redundancy != S3RedundancyReduced {
		t.Errorf("bad redundancy: %q", *s3CreateResp1.Redundancy)
	}
	if *s3CreateResp1.MessageType != "classic" {
		t.Errorf("bad message_type: %q", *s3CreateResp1.MessageType)
	}
	if *s3CreateResp1.Placement != "none" {
		t.Errorf("bad placement: %q", *s3CreateResp1.Placement)
	}
	if *s3CreateResp1.ResponseCondition != "" {
		t.Errorf("bad response_condition: %q", *s3CreateResp1.ResponseCondition)
	}
	if *s3CreateResp1.PublicKey != pgpPublicKey() {
		t.Errorf("bad public_key: %q", *s3CreateResp1.PublicKey)
	}
	if *s3CreateResp1.ServerSideEncryption != S3ServerSideEncryptionKMS {
		t.Errorf("bad server_side_encryption: %q", *s3CreateResp1.ServerSideEncryption)
	}
	if *s3CreateResp1.ServerSideEncryptionKMSKeyID != "1234" {
		t.Errorf("bad server_side_encryption_kms_key_id: %q", *s3CreateResp1.ServerSideEncryptionKMSKeyID)
	}
	if *s3CreateResp1.ACL != S3AccessControlListPrivate {
		t.Errorf("bad acl: %s", *s3CreateResp1.ACL)
	}
	if *s3CreateResp1.FileMaxBytes != MiB {
		t.Errorf("bad file_max_bytes: %d", *s3CreateResp1.FileMaxBytes)
	}
	if *s3CreateResp2.FileMaxBytes != 10*MiB {
		t.Errorf("bad file_max_bytes: %d", *s3CreateResp2.FileMaxBytes)
	}
	if *s3CreateResp3.CompressionCodec != "snappy" {
		t.Errorf("bad compression_codec: %q", *s3CreateResp1.CompressionCodec)
	}
	if *s3CreateResp3.GzipLevel != 0 {
		t.Errorf("bad gzip_level: %d", *s3CreateResp1.GzipLevel)
	}
	if *s3CreateResp3.Redundancy != S3RedundancyStandardIA {
		t.Errorf("bad acl: %s", *s3CreateResp3.Redundancy)
	}
	if *s3CreateResp3.ACL != S3AccessControlListBucketOwnerFullControl {
		t.Errorf("bad acl: %s", *s3CreateResp3.ACL)
	}
	if *s3CreateResp3.FileMaxBytes != 0 {
		t.Errorf("bad file_max_bytes: %d", *s3CreateResp3.FileMaxBytes)
	}
	if s3CreateResp4.AccessKey != nil {
		t.Errorf("bad access_key: %q", *s3CreateResp4.AccessKey)
	}
	if s3CreateResp4.SecretKey != nil {
		t.Errorf("bad secret_key: %q", *s3CreateResp4.SecretKey)
	}
	if *s3CreateResp4.IAMRole != "arn:aws:iam::123456789012:role/S3Access" {
		t.Errorf("bad iam_role: %q", *s3CreateResp4.IAMRole)
	}
	if *s3CreateResp4.Redundancy != S3RedundancyStandard {
		t.Errorf("bad acl: %s", *s3CreateResp4.Redundancy)
	}
	if s3CreateResp4.ACL != nil {
		t.Errorf("bad acl: %s", *s3CreateResp4.ACL)
	}
	if *s3CreateResp4.FileMaxBytes != 10*MiB {
		t.Errorf("bad file_max_bytes: %d", *s3CreateResp4.FileMaxBytes)
	}

	// List
	var s3s []*S3
	Record(t, "s3s/compute/list", func(c *Client) {
		s3s, err = c.ListS3s(context.TODO(), &ListS3sInput{
			ServiceID:      TestComputeServiceID,
			ServiceVersion: *tv.Number,
		})
	})
	if err != nil {
		t.Fatal(err)
	}
	if len(s3s) < 1 {
		t.Errorf("bad s3s: %v", s3s)
	}

	// Get
	var s3GetResp, s3GetResp2 *S3
	Record(t, "s3s/compute/get", func(c *Client) {
		s3GetResp, err = c.GetS3(context.TODO(), &GetS3Input{
			ServiceID:      TestComputeServiceID,
			ServiceVersion: *tv.Number,
			Name:           "test-s3",
		})
	})
	if err != nil {
		t.Fatal(err)
	}

	// Request the configuration using the IAM role
	Record(t, "s3s/compute/get2", func(c *Client) {
		s3GetResp2, err = c.GetS3(context.TODO(), &GetS3Input{
			ServiceID:      TestComputeServiceID,
			ServiceVersion: *tv.Number,
			Name:           "test-s3-4",
		})
	})
	if err != nil {
		t.Fatal(err)
	}

	if *s3CreateResp1.Name != *s3GetResp.Name {
		t.Errorf("bad name: %q", *s3CreateResp1.Name)
	}
	if *s3CreateResp1.BucketName != *s3GetResp.BucketName {
		t.Errorf("bad bucket_name: %q", *s3CreateResp1.BucketName)
	}
	if *s3CreateResp1.AccessKey != *s3GetResp.AccessKey {
		t.Errorf("bad access_key: %q", *s3CreateResp1.AccessKey)
	}
	if *s3CreateResp1.SecretKey != *s3GetResp.SecretKey {
		t.Errorf("bad secret_key: %q", *s3CreateResp1.SecretKey)
	}
	if s3CreateResp1.IAMRole != s3GetResp.IAMRole {
		t.Errorf("bad iam_role: %q", *s3CreateResp1.IAMRole)
	}
	if *s3CreateResp1.Domain != *s3GetResp.Domain {
		t.Errorf("bad domain: %q", *s3CreateResp1.Domain)
	}
	if *s3CreateResp1.Path != *s3GetResp.Path {
		t.Errorf("bad path: %q", *s3CreateResp1.Path)
	}
	if *s3CreateResp1.Period != *s3GetResp.Period {
		t.Errorf("bad period: %d", *s3CreateResp1.Period)
	}
	if *s3CreateResp1.CompressionCodec != *s3GetResp.CompressionCodec {
		t.Errorf("bad compression_codec: %q", *s3CreateResp1.CompressionCodec)
	}
	if *s3CreateResp1.GzipLevel != *s3GetResp.GzipLevel {
		t.Errorf("bad gzip_level: %d", *s3CreateResp1.GzipLevel)
	}
	if *s3CreateResp1.Format != *s3GetResp.Format {
		t.Errorf("bad format: %q", *s3CreateResp1.Format)
	}
	if *s3CreateResp1.FormatVersion != *s3GetResp.FormatVersion {
		t.Errorf("bad format_version: %d", *s3CreateResp1.FormatVersion)
	}
	if *s3CreateResp1.TimestampFormat != *s3GetResp.TimestampFormat {
		t.Errorf("bad timestamp_format: %q", *s3CreateResp1.TimestampFormat)
	}
	if *s3CreateResp1.Redundancy != *s3GetResp.Redundancy {
		t.Errorf("bad redundancy: %q", *s3CreateResp1.Redundancy)
	}
	if *s3CreateResp1.Placement != *s3GetResp.Placement {
		t.Errorf("bad placement: %q", *s3CreateResp1.Placement)
	}
	if *s3CreateResp1.ResponseCondition != "" {
		t.Errorf("bad response_condition: %q", *s3CreateResp1.ResponseCondition)
	}
	if *s3CreateResp1.PublicKey != pgpPublicKey() {
		t.Errorf("bad public_key: %q", *s3CreateResp1.PublicKey)
	}
	if *s3CreateResp1.ServerSideEncryption != *s3GetResp.ServerSideEncryption {
		t.Errorf("bad server_side_encryption: %q", *s3CreateResp1.ServerSideEncryption)
	}
	if *s3CreateResp1.ServerSideEncryptionKMSKeyID != *s3GetResp.ServerSideEncryptionKMSKeyID {
		t.Errorf("bad server_side_encryption_kms_key_id: %q", *s3CreateResp1.ServerSideEncryptionKMSKeyID)
	}
	if *s3CreateResp1.ACL != *s3GetResp.ACL {
		t.Errorf("bad acl: %s", *s3CreateResp1.ACL)
	}
	if s3CreateResp4.AccessKey != s3GetResp2.AccessKey {
		t.Errorf("bad access_key: %q", *s3CreateResp4.AccessKey)
	}
	if s3CreateResp4.SecretKey != s3GetResp2.SecretKey {
		t.Errorf("bad secret_key: %q", *s3CreateResp4.SecretKey)
	}
	if *s3CreateResp4.IAMRole != *s3GetResp2.IAMRole {
		t.Errorf("bad iam_role: %q", *s3CreateResp4.IAMRole)
	}
	if *s3CreateResp4.Redundancy != *s3GetResp2.Redundancy {
		t.Errorf("bad redundancy: %q", *s3CreateResp4.Redundancy)
	}
	if s3CreateResp4.ACL != s3GetResp2.ACL {
		t.Errorf("bad acl: %s", *s3CreateResp4.ACL)
	}

	// Update
	var s3UpdateResp1, s3UpdateResp2, s3UpdateResp3, s3UpdateResp4, s3UpdateResp5 *S3
	Record(t, "s3s/compute/update", func(c *Client) {
		s3UpdateResp1, err = c.UpdateS3(context.TODO(), &UpdateS3Input{
			ServiceID:        TestComputeServiceID,
			ServiceVersion:   *tv.Number,
			Name:             "test-s3",
			NewName:          new("new-test-s3"),
			PublicKey:        new(pgpPublicKeyUpdate()),
			CompressionCodec: new("zstd"),
			FileMaxBytes:     new(5 * MiB),
			ProcessingRegion: new("eu"),
			Placement:        NewNullable("none"),
		})
	})
	if err != nil {
		t.Fatal(err)
	}
	if *s3UpdateResp1.Placement != "none" {
		t.Errorf("bad placement: %q", *s3UpdateResp1.Placement)
	}

	// Test that CompressionCodec can be set for a an endpoint where
	// GzipLevel was specified at creation time.
	Record(t, "s3s/compute/update2", func(c *Client) {
		s3UpdateResp2, err = c.UpdateS3(context.TODO(), &UpdateS3Input{
			ServiceID:        TestComputeServiceID,
			ServiceVersion:   *tv.Number,
			Name:             "test-s3-2",
			CompressionCodec: new("zstd"),
			Placement:        NewNullable("none"),
		})
	})
	if err != nil {
		t.Fatal(err)
	}
	if *s3UpdateResp2.Placement != "none" {
		t.Errorf("bad placement: %q", *s3UpdateResp2.Placement)
	}

	// Test that GzipLevel can be set for an endpoint where CompressionCodec
	// was set at creation time.
	Record(t, "s3s/compute/update3", func(c *Client) {
		s3UpdateResp3, err = c.UpdateS3(context.TODO(), &UpdateS3Input{
			ServiceID:      TestComputeServiceID,
			ServiceVersion: *tv.Number,
			Name:           "test-s3-3",
			GzipLevel:      new(9),
			Placement:      NewNullable("none"),
		})
	})
	if err != nil {
		t.Fatal(err)
	}
	if *s3UpdateResp3.Placement != "none" {
		t.Errorf("bad placement: %q", *s3UpdateResp3.Placement)
	}

	// Test that a configuration using an access key/secret key can be
	// updated to use IAM role.
	Record(t, "s3s/compute/update4", func(c *Client) {
		s3UpdateResp4, err = c.UpdateS3(context.TODO(), &UpdateS3Input{
			ServiceID:      TestComputeServiceID,
			ServiceVersion: *tv.Number,
			Name:           "new-test-s3",
			AccessKey:      new(""),
			SecretKey:      new(""),
			IAMRole:        new("arn:aws:iam::123456789012:role/S3Access"),
			Placement:      NewNullable("none"),
		})
	})
	if err != nil {
		t.Fatal(err)
	}
	if *s3UpdateResp4.Placement != "none" {
		t.Errorf("bad placement: %q", *s3UpdateResp4.Placement)
	}

	// Test that a configuration using an IAM role can be updated to use
	// access key/secret key.
	Record(t, "s3s/compute/update5", func(c *Client) {
		s3UpdateResp5, err = c.UpdateS3(context.TODO(), &UpdateS3Input{
			ServiceID:      TestComputeServiceID,
			ServiceVersion: *tv.Number,
			Name:           "test-s3-4",
			AccessKey:      new("AKIAIOSFODNN7EXAMPLE"),
			SecretKey:      new("wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY"),
			IAMRole:        new(""),
			Placement:      NewNullable("none"),
		})
	})
	if err != nil {
		t.Fatal(err)
	}
	if *s3UpdateResp5.Placement != "none" {
		t.Errorf("bad placement: %q", *s3UpdateResp5.Placement)
	}

	// Test that an invalid IAM role ARN is rejected. This case is expected
	// to fail.
	Record(t, "s3s/compute/update6", func(c *Client) {
		_, err = c.UpdateS3(context.TODO(), &UpdateS3Input{
			ServiceID:      TestComputeServiceID,
			ServiceVersion: *tv.Number,
			Name:           "test-s3",
			IAMRole:        new("badarn"),
			Placement:      NewNullable("none"),
		})
	})
	if err == nil {
		t.Fatal(err)
	}

	if *s3UpdateResp1.Name != "new-test-s3" {
		t.Errorf("bad name: %q", *s3UpdateResp1.Name)
	}
	if *s3UpdateResp1.PublicKey != pgpPublicKeyUpdate() {
		t.Errorf("bad public_key: %q", *s3UpdateResp1.PublicKey)
	}
	if *s3UpdateResp1.CompressionCodec != "zstd" {
		t.Errorf("bad compression_codec: %q", *s3UpdateResp1.CompressionCodec)
	}
	if s3UpdateResp1.GzipLevel != nil {
		t.Errorf("bad gzip_level: %d", *s3UpdateResp1.GzipLevel)
	}
	if *s3UpdateResp1.FileMaxBytes != 5*MiB {
		t.Errorf("bad file_max_bytes: %d", *s3UpdateResp1.FileMaxBytes)
	}
	if *s3UpdateResp1.ProcessingRegion != "eu" {
		t.Errorf("bad log_processing_region: %q", *s3UpdateResp1.ProcessingRegion)
	}
	if *s3UpdateResp2.CompressionCodec != "zstd" {
		t.Errorf("bad compression_codec: %q", *s3UpdateResp2.CompressionCodec)
	}
	if *s3UpdateResp2.GzipLevel != 0 {
		t.Errorf("bad gzip_level: %d", *s3UpdateResp2.GzipLevel)
	}
	if s3UpdateResp3.CompressionCodec != nil {
		t.Errorf("bad compression_codec: %q", *s3UpdateResp3.CompressionCodec)
	}
	if *s3UpdateResp3.GzipLevel != 9 {
		t.Errorf("bad gzip_level: %d", *s3UpdateResp3.GzipLevel)
	}
	if *s3UpdateResp4.AccessKey != "" {
		t.Errorf("bad access_key: %q", *s3UpdateResp4.AccessKey)
	}
	if *s3UpdateResp4.SecretKey != "" {
		t.Errorf("bad secret_key: %q", *s3UpdateResp4.SecretKey)
	}
	if *s3UpdateResp4.IAMRole != "arn:aws:iam::123456789012:role/S3Access" {
		t.Errorf("bad iam_role: %q", *s3UpdateResp4.IAMRole)
	}
	if *s3UpdateResp5.AccessKey != "AKIAIOSFODNN7EXAMPLE" { //nolint: gosec
		t.Errorf("bad access_key: %q", *s3UpdateResp5.AccessKey)
	}
	if *s3UpdateResp5.SecretKey != "wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY" {
		t.Errorf("bad secret_key: %q", *s3UpdateResp5.SecretKey)
	}
	if *s3UpdateResp5.IAMRole != "" {
		t.Errorf("bad iam_role: %q", *s3UpdateResp5.IAMRole)
	}

	// Delete
	Record(t, "s3s/compute/delete", func(c *Client) {
		err = c.DeleteS3(context.TODO(), &DeleteS3Input{
			ServiceID:      TestComputeServiceID,
			ServiceVersion: *tv.Number,
			Name:           "new-test-s3",
		})
	})
	if err != nil {
		t.Fatal(err)
	}
}

func TestClient_ListS3s_validation(t *testing.T) {
	var err error

	_, err = TestClient.ListS3s(context.TODO(), &ListS3sInput{
		ServiceID: "",
	})
	if !errors.Is(err, ErrMissingServiceID) {
		t.Errorf("bad error: %s", err)
	}

	_, err = TestClient.ListS3s(context.TODO(), &ListS3sInput{
		ServiceID:      "foo",
		ServiceVersion: 0,
	})
	if !errors.Is(err, ErrMissingServiceVersion) {
		t.Errorf("bad error: %s", err)
	}
}

func TestClient_CreateS3_validation(t *testing.T) {
	var err error

	_, err = TestClient.CreateS3(context.TODO(), &CreateS3Input{
		Name:                         new("test-service"),
		ServerSideEncryption:         new(S3ServerSideEncryptionKMS),
		ServerSideEncryptionKMSKeyID: new("1234"),
		ServiceVersion:               1,
	})
	if !errors.Is(err, ErrMissingServiceID) {
		t.Errorf("bad error: %s", err)
	}

	_, err = TestClient.CreateS3(context.TODO(), &CreateS3Input{
		Name:                         new("test-service"),
		ServerSideEncryption:         new(S3ServerSideEncryptionKMS),
		ServerSideEncryptionKMSKeyID: new("1234"),
		ServiceID:                    "foo",
	})
	if !errors.Is(err, ErrMissingServiceVersion) {
		t.Errorf("bad error: %s", err)
	}

	_, err = TestClient.CreateS3(context.TODO(), &CreateS3Input{
		Name:                         new("test-service"),
		ServerSideEncryption:         new(S3ServerSideEncryptionKMS),
		ServerSideEncryptionKMSKeyID: new(""),
		ServiceID:                    "foo",
		ServiceVersion:               1,
	})
	if !errors.Is(err, ErrMissingServerSideEncryptionKMSKeyID) {
		t.Errorf("bad error: %s", err)
	}
}

func TestClient_GetS3_validation(t *testing.T) {
	var err error

	_, err = TestClient.GetS3(context.TODO(), &GetS3Input{
		ServiceID:      "foo",
		ServiceVersion: 1,
	})
	if !errors.Is(err, ErrMissingName) {
		t.Errorf("bad error: %s", err)
	}

	_, err = TestClient.GetS3(context.TODO(), &GetS3Input{
		Name:           "test",
		ServiceVersion: 1,
	})
	if !errors.Is(err, ErrMissingServiceID) {
		t.Errorf("bad error: %s", err)
	}

	_, err = TestClient.GetS3(context.TODO(), &GetS3Input{
		Name:      "test",
		ServiceID: "foo",
	})
	if !errors.Is(err, ErrMissingServiceVersion) {
		t.Errorf("bad error: %s", err)
	}
}

func TestClient_UpdateS3_validation(t *testing.T) {
	var err error

	_, err = TestClient.UpdateS3(context.TODO(), &UpdateS3Input{
		ServiceID:      "foo",
		ServiceVersion: 1,
	})
	if !errors.Is(err, ErrMissingName) {
		t.Errorf("bad error: %s", err)
	}

	_, err = TestClient.UpdateS3(context.TODO(), &UpdateS3Input{
		Name:           "test",
		ServiceVersion: 1,
	})
	if !errors.Is(err, ErrMissingServiceID) {
		t.Errorf("bad error: %s", err)
	}

	_, err = TestClient.UpdateS3(context.TODO(), &UpdateS3Input{
		Name:      "test",
		ServiceID: "foo",
	})
	if !errors.Is(err, ErrMissingServiceVersion) {
		t.Errorf("bad error: %s", err)
	}

	_, err = TestClient.UpdateS3(context.TODO(), &UpdateS3Input{
		Name:                         "test-service",
		ServerSideEncryption:         new(S3ServerSideEncryptionKMS),
		ServerSideEncryptionKMSKeyID: new(""),
		ServiceID:                    "foo",
		ServiceVersion:               1,
	})
	if !errors.Is(err, ErrMissingServerSideEncryptionKMSKeyID) {
		t.Errorf("bad error: %s", err)
	}
}

func TestClient_DeleteS3_validation(t *testing.T) {
	var err error

	err = TestClient.DeleteS3(context.TODO(), &DeleteS3Input{
		ServiceID:      "foo",
		ServiceVersion: 1,
	})
	if !errors.Is(err, ErrMissingName) {
		t.Errorf("bad error: %s", err)
	}

	err = TestClient.DeleteS3(context.TODO(), &DeleteS3Input{
		Name:           "test",
		ServiceVersion: 1,
	})
	if !errors.Is(err, ErrMissingServiceID) {
		t.Errorf("bad error: %s", err)
	}

	err = TestClient.DeleteS3(context.TODO(), &DeleteS3Input{
		Name:      "test",
		ServiceID: "foo",
	})
	if !errors.Is(err, ErrMissingServiceVersion) {
		t.Errorf("bad error: %s", err)
	}
}
