...
func Test_WorkspaceAlerts(t *testing.T) {
	...
	if *getTestWorkspaceAlert.Config.Host != *testConfig.Host {
		t.Errorf("unexpected workspace alert config host: got %v, expected %v", *getTestWorkspaceAlert.Config.Host, *testConfig.Host)
	}
	...
	if *getTestWorkspaceAlert.Config.Project != *testConfig.Project {
		t.Errorf("unexpected workspace alert config project: got %v, expected %v", *getTestWorkspaceAlert.Config.Project, *testConfig.Project)
	}
	if *getTestWorkspaceAlert.Config.Username != *testConfig.Username {
		t.Errorf("unexpected workspace alert config username: got %v, expected %v", *getTestWorkspaceAlert.Config.Username, *testConfig.Username)
	}
	if *getTestWorkspaceAlert.Config.IssueType != *testConfig.IssueType {
		t.Errorf("unexpected workspace alert config issue type: got %v, expected %v", *getTestWorkspaceAlert.Config.IssueType, *testConfig.IssueType)
	}
	...
	if *updateWorkspaceAlert.Config.Host != *updatedConfig.Host {
		t.Errorf("unexpected updated workspace alert config host: got %v, expected %v", *updateWorkspaceAlert.Config.Host, *updatedConfig.Host)
	}
	...
	if *updateWorkspaceAlert.Config.Project != *updatedConfig.Project {
		t.Errorf("unexpected updated workspace alert config project: got %v, expected %v", *updateWorkspaceAlert.Config.Project, *updatedConfig.Project)
	}
	if *updateWorkspaceAlert.Config.Username != *updatedConfig.Username {
		t.Errorf("unexpected updated workspace alert config username: got %v, expected %v", *updateWorkspaceAlert.Config.Username, *updatedConfig.Username)
	}
	if *updateWorkspaceAlert.Config.IssueType != *updatedConfig.IssueType {
		t.Errorf("unexpected updated workspace alert config issue type: got %v, expected %v", *updateWorkspaceAlert.Config.IssueType, *updatedConfig.IssueType)
	}
