...
// CreateAction represents an action taken when a rule's conditions are met.
type CreateAction struct {
	// Type specifies the action type (e.g., "block", "exclude_signal") (required).
	Type *string `json:"type"`
	// Signal is the signal name used only for the "exclude_signal" action type.
	Signal *string `json:"signal,omitempty"`
	// RedirectURL specifies the target URL when redirecting the request.
	RedirectURL *string `json:"redirect_url,omitempty"`
	// ResponseCode is the HTTP status code returned during redirection (e.g., 301, 302).
	ResponseCode *int `json:"response_code,omitempty"`
}

// CreateCondition defines a single condition.
type CreateCondition struct {
	// Type specifies the condition type (must be "single") (required).
	Type *string `json:"type"`
	// Field is the name of the field to be evaluated (e.g., "ip", "path") (required).
	Field *string `json:"field"`
	// Operator determines how the field is evaluated (e.g., "equals", "contains") (required).
	Operator *string `json:"operator"`
	// Value is the value against which the field is compared (required).
	Value *string `json:"value"`
}

// CreateGroupCondition defines a group of conditions with a logical operator.
type CreateGroupCondition struct {
	// Type specifies the condition group type (must be "group") (required).
	Type *string `json:"type"`
	// GroupOperator is the logical operator used to evaluate the conditions ("any" or "all") (required).
	GroupOperator *string `json:"group_operator"`
	// Conditions is the list of single conditions to evaluate within the group (required).
