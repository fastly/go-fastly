...
// Threshold is the API response structure for the create, update, and get operations.
type Threshold struct {
	// Action to take when threshold is exceeded. Must be one of `block` or `log`.
	...
	// Duration is the duration the action is in place. Default duration is 0.
	...
	// Limit is the threshold limit. Must be between than 1 and less than 10000 inclusive. Required.
	Limit int `json:"limit"`
	// Name is the threshold name.
	Name string `json:"name"`
	// Signal is the name of the signal this threshold is acting on. Required.
	Signal string `json:"signal"`
	// ThresholdID is the threshold identifier (UUID).
	ThresholdID string `json:"id"`
}

// Thresholds is the API response structure for the list Thresholds operation.
...
// MetaThresholds is a subset of the list thresholds response structure.
