...
type ListInput struct {
	// From represents the start of a date-time range, expressed
	// in RFC 3339 format (required).
	From *string
	...
	Limit *int
	// Page is the page number of the collection to
	// request. [Default 0]
	Page *int
	// Signal filters the list of events based on signal.
	Signal *string
	// Status filters the list of events based on status. Must be
	// one of `active` or `expired`.
	Status *string
	// To represents the end of a date-time range and must be
	// older than from, expressed in RFC 3339 format.
	To *string
