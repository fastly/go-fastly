...
func Test_Alerts(t *testing.T) {
	...
	testConfig := &CreateConfig{
		Webhook: new("https://example.webhook.office.com/webhookb2/test"),
	}
	...
	updatedConfig := &UpdateConfig{
		Webhook: new("https://updated.webhook.office.com/webhookb2/test"),
	}
	updatedEvent := "flag"
...
func TestClient_CreateAlert_validation(t *testing.T) {
	...
	_, err = Create(context.TODO(), fastly.TestClient, &CreateInput{
		Config:      &CreateConfig{Webhook: new("https://example.webhook.office.com/webhookb2/...")},
		Events:      nil,
		WorkspaceID: new(fastly.TestNGWAFWorkspaceID),
	})
