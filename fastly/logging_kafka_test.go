...
func TestClient_Kafkas(t *testing.T) {
	...
	Record(t, "kafkas/create", func(c *Client) {
		k, err = c.CreateKafka(context.TODO(), &CreateKafkaInput{
			...
			Brokers:          new("192.168.1.1,192.168.1.2"),
			CompressionCodec: new("lz4"),
			Format:           new("%h %l %u %t \"%r\" %>s %b"),
			FormatVersion:    new(2),
			Name:             new("test-kafka"),
			ParseLogKeyvals:  new(Compatibool(true)),
			Password:         new("deadbeef"),
			Placement:        new("none"),
			RequestMaxBytes:  new(requestMaxBytes),
			RequiredACKs:     new("-1"),
	...
	Record(t, "kafkas/update", func(c *Client) {
		uk, err = c.UpdateKafka(context.TODO(), &UpdateKafkaInput{
			...
			ServiceVersion:   *tv.Number,
			Name:             "test-kafka",
			NewName:          new("new-test-kafka"),
			Topic:            new("new-kafka-topic"),
			ProcessingRegion: new("eu"),
			Placement:        NullValue[string](),
			ParseLogKeyvals:  new(Compatibool(false)),
...
func TestClient_Kafkas_Compute(t *testing.T) {
	t.Parallel()

	var err error
	var tv *Version
	Record(t, "kafkas/compute/version", func(c *Client) {
	...
	// Create
	var k *Kafka
	Record(t, "kafkas/compute/create", func(c *Client) {
		k, err = c.CreateKafka(context.TODO(), &CreateKafkaInput{
			AuthMethod:       new("scram-sha-512"),
			Brokers:          new("192.168.1.1,192.168.1.2"),
			CompressionCodec: new("lz4"),
	...
	// Ensure deleted
	defer func() {
		Record(t, "kafkas/compute/cleanup", func(c *Client) {
			_ = c.DeleteKafka(context.TODO(), &DeleteKafkaInput{
				ServiceID:      TestComputeServiceID,
				ServiceVersion: *tv.Number,
				Name:           "test-kafka",
	...
	// List
	var ks []*Kafka
	Record(t, "kafkas/compute/list", func(c *Client) {
		ks, err = c.ListKafkas(context.TODO(), &ListKafkasInput{
			ServiceID:      TestComputeServiceID,
			ServiceVersion: *tv.Number,
		})
	...
	// Get
	var nk *Kafka
	Record(t, "kafkas/compute/get", func(c *Client) {
		nk, err = c.GetKafka(context.TODO(), &GetKafkaInput{
			ServiceID:      TestComputeServiceID,
			ServiceVersion: *tv.Number,
			Name:           "test-kafka",
	...
	// Update
	var uk *Kafka
	Record(t, "kafkas/compute/update", func(c *Client) {
		uk, err = c.UpdateKafka(context.TODO(), &UpdateKafkaInput{
			ServiceID:        TestComputeServiceID,
			ServiceVersion:   *tv.Number,
			Name:             "test-kafka",
	...
	// Delete
	Record(t, "kafkas/compute/delete", func(c *Client) {
		err = c.DeleteKafka(context.TODO(), &DeleteKafkaInput{
			ServiceID:      TestComputeServiceID,
			ServiceVersion: *tv.Number,
			Name:           "new-test-kafka",
