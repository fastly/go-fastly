...
func TestClient_GCSs(t *testing.T) {
	...
	if gcs.CompressionCodec != "zstd" {
		t.Errorf("bad compression_codec: %q", gcs.CompressionCodec)
	}
	if gcs.GzipLevel != 0 {
	...
	if gcs.CompressionCodec != ngcs.CompressionCodec {
		t.Errorf("bad compression_codec: %q", gcs.CompressionCodec)
	}
	...
	if ugcs.CompressionCodec != "" {
		t.Errorf("bad compression_codec: %q", ugcs.CompressionCodec)
	}
	if ugcs.GzipLevel != 9 {
		t.Errorf("bad gzip_level: %q", ugcs.GzipLevel)
	}
