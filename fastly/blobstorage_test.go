...
func TestClient_BlobStorages(t *testing.T) {
	...
	if bs.SASToken != "sv=2015-04-05&ss=b&srt=sco&sp=rw&se=2030-07-21T18%3A00%3A00Z&sig=3ABdLOJZosCp0o491T%2BqZGKIhafF1nlM3MzESDDD3Gg%3D" {
	...
	if bs.CompressionCodec != "zstd" {
		t.Errorf("bad compression_codec: %q", bs.CompressionCodec)
	}
	if bs.GzipLevel != 0 {
	...
	if bs.CompressionCodec != nbs.CompressionCodec {
		t.Errorf("bad compression_codec: %q", bs.CompressionCodec)
	}
	...
	if ubs.CompressionCodec != "snappy" {
		t.Errorf("bad compression_codec: %q", ubs.CompressionCodec)
	}
