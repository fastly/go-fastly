...
type BulkUpdateDiscoveredStatusInput struct {
	...
	// OperationIDs is the list of operation IDs to update (required).
...
func BulkUpdateDiscoveredStatus(ctx context.Context, c *fastly.Client, i *BulkUpdateDiscoveredStatusInput) (*BulkOperationResultsResponse, error) {
	...
	body := &DiscoveredOperationBulkStatusUpdate{
		OperationIDs: i.OperationIDs,
		Status:       i.Status,
	}

	resp, err := c.PatchJSON(ctx, path, body, fastly.CreateRequestOptions())
	if err != nil {
		return nil, err
	}
