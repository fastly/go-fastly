...
// Get gets the status of the BrotliCompression product on the service.
func Get(c *fastly.Client, serviceID string) (*fastly.ProductEnablement, error) {
	if serviceID == "" {
		return nil, fastly.ErrMissingServiceID
	}

	path := fastly.ToSafeURL("enabled-products", "brotli_compression", "services", serviceID)
...
// Enable enables the BrotliCompression product on the service.
...
// Disable disables the BrotliCompression product on the service.
